...
class AcarreosMetroWidget extends StatefulWidget {
  ...
  @override
  _AcarreosMetroWidgetState createState() => _AcarreosMetroWidgetState();
}

class _AcarreosMetroWidgetState extends State<AcarreosMetroWidget> {
  List<AcarreoMetro> acarreos = [];

  @override
  void initState() {
    super.initState();
    _cargarAcarreos();
    // dev.log('ACARREOS Metro' + acarreos.toString());
  }

  void _cargarAcarreos() {
    setState(() {
      acarreos = PreferenceProvider.getAcarreosMetro('acarreos_metro');
    });
  }

  void _agregarAcarreo(AcarreoMetro nuevoAcarreo) {
    setState(() {
      PreferenceProvider.addAcarreoMetro('acarreos_metro', nuevoAcarreo);
      acarreos = PreferenceProvider.getAcarreosMetro('acarreos_metro');
    });
  }
  ...
  void _eliminarAcarreo(int index) {
    setState(() {
      PreferenceProvider.removeAcarreoMetro('acarreos_metro', index);
      acarreos = PreferenceProvider.getAcarreosMetro('acarreos_metro');
    });
  }
  ...
  void _editarAcarreo(int index) async {
    ...
    if (resultado != null && resultado is AcarreoMetro) {
      setState(() {
        PreferenceProvider.updateAcarreoMetro(
            'acarreos_metro', index, resultado);
        acarreos = PreferenceProvider.getAcarreosMetro('acarreos_metro');
      });
    }
  }
